 ...
 * This isn't for use outside of the Vertext Framework implementation.
 */
export interface VertexCore {
    read<T>(code: (tx: Transaction) => Promise<T>): Promise<T>;
    _restrictedWrite<T>(code: (tx: Transaction) => Promise<T>): Promise<T>;
